...
export class NotificationSettingsRenderService extends AbstractRenderService<NotificationSettings> {
  ...
  private initDocumentElements(): void {
    this.regularNotificationToggle = document.getElementById(
      "notification-toggle"
    ) as HTMLInputElement;
    this.twoWeeklyFrequencyRb = document.getElementById(
      "2-weekly-rb"
    ) as HTMLInputElement;
    this.fourWeeklyFrequencyRb = document.getElementById(
      "4-weekly-rb"
    ) as HTMLInputElement;
  ...
  private addEventListener(): void {
    ...
    this.twoWeeklyFrequencyRb.addEventListener(
      "change",
      this.onAnyValueChange.bind(this)
    );
    this.fourWeeklyFrequencyRb.addEventListener(
      "change",
      this.onAnyValueChange.bind(this)
    );
    this.releaseDateNotificationToggle.addEventListener(
      "change",
      this.onAnyValueChange.bind(this)
    );
    this.announcementDateNotificationToggle.addEventListener(
      "change",
      this.onAnyValueChange.bind(this)
    );
  }
  ...
  protected onRendering(notificationSettings: NotificationSettings): void {
    ...
    this.twoWeeklyFrequencyRb.disabled = !this.regularNotificationToggle
      .checked;
    this.fourWeeklyFrequencyRb.disabled = !this.regularNotificationToggle
      .checked;
    this.twoWeeklyFrequencyRb.checked =
      notificationSettings.frequencyInWeeks === 2;
    this.fourWeeklyFrequencyRb.checked =
      notificationSettings.frequencyInWeeks === 4;
    this.releaseDateNotificationToggle.checked =
      notificationSettings.notificationAtReleaseDate;
    this.announcementDateNotificationToggle.checked =
      notificationSettings.notificationAtAnnouncementDate;
  }
  ...
  private persistCurrentSettings(): void {
    this.notificationSettingsRestClient
      .updateNotificationSettings({
        ...
        notificationAtAnnouncementDate: this.announcementDateNotificationToggle
          .checked,
      })
      .catch((response) => {
