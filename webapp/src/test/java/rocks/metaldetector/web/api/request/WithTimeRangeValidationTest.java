...
class WithTimeRangeValidationTest implements WithAssertions {
  ...
  private static Stream<Arguments> releaseRequestProviderAllDates() {
    ...
    return Stream.of(
            ...
            Arguments.of(new ReleasesRequest(null, now, Collections.emptyList()), true),
            Arguments.of(new ReleasesRequest(now, now, Collections.emptyList()), true),
            Arguments.of(new ReleasesRequest(now, now.plusDays(1), Collections.emptyList()), true),
            Arguments.of(new ReleasesRequest(now.plusDays(1), now, Collections.emptyList()), false)
    );
  }
